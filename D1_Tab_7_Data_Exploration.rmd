...
Using the `trees2` data frame, which fixed the decay class column by making the `DecayClassCode_num` field numeric, we're now going to drop visits that were for QAQC using a new base R function called `subset()`. I will also show the bracket approach. 

<p class = 'code'>Remove records with "PM" as DecayClassCode </p>
```{r tr3}
trees3 <- subset(trees2, IsQAQC == FALSE) # Note the importance of FALSE all caps
trees3 <- subset(trees2, IsQAQC != TRUE) # equivalent
trees3 <- trees2[trees2$IsQAQC == FALSE,] #equivalent but not as easy to follow
```
</details><br>
