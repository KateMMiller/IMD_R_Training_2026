...
<strong>CHALLENGE: How would you look at the the first 4 even rows (2, 4, 6, 8)?</strong>
...
<div class="toggle"><button>View output Option 1</button>
```{r rows2, echo = F}
sort(unique(ACAD_wetland[, "Latin_Name"]))
```
</div>
...
```{r rows2b1, results = "hide"}
sort(unique(ACAD_wetland$Latin_Name)) # equivalent
```

<div class="toggle"><button>View output Option 2</button>
```{r rows2b, echo = F}
sort(unique(ACAD_wetland$Latin_Name)) # equivalent
```
</div>
