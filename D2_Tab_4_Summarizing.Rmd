...
<strong>CHALLENGE: Using the ACAD_wetland data, sum the percent cover of native vs. invasive species per plot (use the Ave_Cov column). Note that Invasive = TRUE is invasive and FALSE is native. </strong>
...
<p class = "code">Most efficient solution figured out during training </p>
```{r relcovnew, eval = T}
...
<p class = 'code'>Original Solution: First sum site-level cover using mutate to return a value for every original row.</p>
```{r wetrelcov, eval = T}
