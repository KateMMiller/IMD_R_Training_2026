...
ACAD_wetland[, "Site_Name"]  # Same as ACAD_wetland$Site_Name
ACAD_wetland[, c("Latin_Name", "Common")]  # Data frame with only Latin Name and Common columns
ACAD_wetland[1:5, "Latin_Name"]  # First five elements of Latin Name column
```

Now that we've learned about vectors, these data frame columns might look familiar. A data frame is just a collection of vectors of the same length, where each vector is a column. Every column has the same number of rows (i.e. it's rectangular)

```{r ColumnsAreVectors}
is.vector(ACAD_wetland$Latin_Name)
class(ACAD_wetland$Latin_Name)
is.vector(ACAD_wetland$PctFreq)
class(ACAD_wetland$PctFreq)

str(ACAD_wetland)  # Check out the abbreviations next to the column names: chr, num, and int. These are the data types of the columns.
```
